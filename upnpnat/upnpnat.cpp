...
#define DEL_PORT_MAPPING_PARAMS "<NewExternalPort>%u</NewExternalPort>\r\n"\
                                "<NewProtocol>%s</NewProtocol>\r\n"        \
...
bool UPNPNAT::discovery()
{
	...
	char* buff = (char*)malloc(MAX_BUFF_SIZE + 1);
...
bool UPNPNAT::del_port_mapping(unsigned short int _port_ex, const char* _protocal)
{
	int ret;
	...
	char* buff = (char*)malloc(MAX_BUFF_SIZE + 1);
	memset(buff, 0, MAX_BUFF_SIZE + 1);
	sprintf(buff, DEL_PORT_MAPPING_PARAMS, _port_ex, _protocal);
	...
	if (response.find(HTTP_OK) == std::string::npos)
	{
		status = NAT_ERROR;
		char temp[100];
		sprintf(temp, "Fail to del port mapping (%hu/%s)\n", _port_ex, _protocal);
		last_error = temp;
		free(buff);
		return false;
	}
...
bool UPNPNAT::add_port_mapping(const char* _description, const char* _destination_ip, unsigned short int _port_ex, unsigned short int _port_in, const char* _protocal, unsigned int leasetime)
{
	int ret;
	...
	char* buff = (char*)malloc(MAX_BUFF_SIZE + 1);
	memset(buff, 0, MAX_BUFF_SIZE + 1);
	sprintf(buff,ADD_PORT_MAPPING_PARAMS,_port_ex,_protocal,_port_in,_destination_ip,_description, leasetime);
	std::string action_params=buff;
